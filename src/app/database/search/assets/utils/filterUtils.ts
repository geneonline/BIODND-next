export const DATE_SELECTION_SEPARATOR = "|";
...
export const isMilestoneDateFilter = (
  filter: FilterConfig | undefined
): boolean => (filter?.type ?? "") === "milestone-date";

const deserializeMilestoneDateSelection = (
  filter: FilterConfig,
...
export const serializeMilestoneDateSelection = (
  selection: DateSelection | null | undefined
): string | null => {
...
export const createSelectionsFromParams = (
...
const buildMilestoneDateClause = (
...
export const buildFilterString = (
  ...
  filterConfig
    .map((filter) => {
      ...
      const joined = values.map((val: string) => `"${val}"`).join(" OR ");
      return `${filter.field}:(${joined})`;
    })
    .filter(Boolean)
    .join(" AND ");

export const normalizeSelectionValue = (value: any): string | null => {
  if (value === null || value === undefined) return null;
  return typeof value === "string" ? value : String(value);
};

const buildMilestoneTagValue = (
  filter: FilterConfig,
  selection: DateSelection | null
): string | null => {
  if (!selection?.field) return null;
...
export const readPromoResultFlag = (): boolean => {
  if (typeof window === "undefined") return false;
  return localStorage.getItem(PROMO_RESULT_STORAGE_KEY) === "true";
};

export const persistPromoResultFlag = (value: boolean) => {
