...
import { DateSelection } from "../../utils/filterUtils";
...
interface MilestoneDateFilterProps {
  label: string;
  options?: Option[];
...
const MilestoneDateFilter = ({
  label,
  options = [],
  selection,
  onChange,
}: MilestoneDateFilterProps) => {
  const startId = useId();
  const endId = useId();

  const normalizedSelection: Partial<DateSelection> = selection ?? {};
