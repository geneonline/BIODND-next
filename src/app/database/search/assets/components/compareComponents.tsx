...
export const StudyTitleCell = ({ value }: { value: string }) => {
...
export const renderFieldValue = (value: any): React.ReactNode => {
...
export const TableBlockA = ({ rows, data }: TableBlockAProps) => {
  const columnCount = data.length;
  const tableStyle = getTableStyle(columnCount);

  return (
    <div
      className="table-scroll-container overflow-x-auto overflow-y-visible scrollbar-hide border border-gray-200"
          ...
          {rows.map(({ label, field, render, useTooltip }) => (
            <tr key={field} className="border-b border-gray-200">
              ...
              {data.map((item, i) => (
                <td
                  key={i}
                  className="bg-primaryBlue-100 p-4 text-text-primary-color text-[16px] font-medium border-l border-gray-200 align-top"
...
export const TableBlock = ({
  ...
  return (
    <div id={id} className="border border-gray-200 rounded-lg relative">
      <div
            ...
            {rows.map(({ label, field, render }) => (
              <tr key={field ?? label} className="border-b border-gray-200">
                ...
                {data.map((item, i) => (
                  <td
                    key={i}
                    className="bg-white p-4 text-text-primary-color border-l border-gray-200 align-top"
                    style={valueCellStyle}
                  >
                    {render
                      ? render(item?.[field], item)
...
export const createPlaceholderData = (length: number) =>
  Array.from({ length }, () => ({}));

export const alignResultsWithIds = (
  ids: string[],
  hits: any[],
  idField: string
) => {
...
export const useComparePageLayout = (
  ...
  useEffect(() => {
    ...
    const titleBars = document.querySelectorAll<HTMLElement>(".table-titlebar");
