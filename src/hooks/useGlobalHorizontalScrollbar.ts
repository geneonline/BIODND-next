...
interface UseGlobalHorizontalScrollbarProps {
  dependencies?: any[];
...
export function useGlobalHorizontalScrollbar({
...
}: UseGlobalHorizontalScrollbarProps = {}) {
  const bottomScrollRef = useRef<HTMLDivElement>(null);
  const [scrollContainers, setScrollContainers] = useState<Element[]>([]);
  ...
  useEffect(() => {
    ...
    const handleScroll = (event: Event) => {
      ...
      targets.forEach((t) => {
        if (t !== target && t.scrollLeft !== scrollLeft) {
