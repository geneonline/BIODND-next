...
const DeleteNeed_popup = ({ popUp, setPopUp, id }) => {
  const { t } = useTranslation();
  const deleteNeedHandler = (e) => {
    e.preventDefault();
    e.stopPropagation();
    axios
      .delete(`${import.meta.env.VITE_API_URL}/needs/${id}.json`, {
        withCredentials: true,
      })
      .then(() => {
        mutate(`${import.meta.env.VITE_API_URL}/needs.json`);
        setPopUp(false);
      })
      .catch(() => {
        // console.log(err);
          ...
          >
            ...
            <div className="flex flex-col mt-12.5 mr-1.5 items-center">
              ...
              <div className="flex space-x-3 pt-8">
                ...
                >
                  {t("user.needs_profile.popup.delete.cancel")}
                </button>
                <button
                  onClick={deleteNeedHandler}
                  className="w-40  bg-db-search text-white hover:bg-main-color rounded-full text-center py-3"
                >
                  {t("user.needs_profile.popup.delete.confirm")}
                </button>
              </div>
            </div>
...
export default DeleteNeed_popup;
