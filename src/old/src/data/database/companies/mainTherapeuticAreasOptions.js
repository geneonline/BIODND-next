export const mainTherapeuticAreasOptions_en = [
  ["Infectious and parasitic diseases", "Infectious and parasitic diseases"],
  ["Neoplasms", "Neoplasms"],
  [
    "Blood and blood-forming organs and certain disorders involving the immune mechanism",
    "Blood and blood-forming organs and certain disorders involving the immune mechanism",
  ],
  [
    "Endocrine, nutritional and metabolic",
    "Endocrine, nutritional and metabolic",
  ],
  ["Mental and behavioral", "Mental and behavioral"],
  ["Eye and adnexa", "Eye and adnexa"],
  ["Ear and mastoid process", "Ear and mastoid process"],
  ["Circulatory system", "Circulatory system"],
  ["Respiratory system", "Respiratory system"],
  ["Digestive system", "Digestive system"],
  ["Skin and subcutaneous tissue", "Skin and subcutaneous tissue"],
  [
    "Musculoskeletal system and connective tissue",
    "Musculoskeletal system and connective tissue",
  ],
  ["Genitourinary system", "Genitourinary system"],
];

export const mainTherapeuticAreasOptions_zh = [
  ["傳染性和寄生蟲病", "Infectious and parasitic diseases"],
  ["腫瘤", "Neoplasms"],
  [
    "血液和造血器官及某些涉及免疫機制的疾病",
    "Blood and blood-forming organs and certain disorders involving the immune mechanism",
  ],
  ["內分泌、營養和代謝", "Endocrine, nutritional and metabolic"],
  ["精神和行為障礙", "Mental and behavioral"],
  ["眼及其附屬器", "Eye and adnexa"],
  ["耳和乳突", "Ear and mastoid process"],
  ["循環系統", "Circulatory system"],
  ["呼吸系統", "Respiratory system"],
  ["消化系統", "Digestive system"],
  ["皮膚和皮下組織", "Skin and subcutaneous tissue"],
  ["肌肉骨骼系統和結締組織", "Musculoskeletal system and connective tissue"],
  ["泌尿生殖系統", "Genitourinary system"],
];
