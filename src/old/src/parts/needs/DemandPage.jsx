...
import { useDemands } from "@/data/api";
...
const DemandPage = () => {
  ...
  const { id } = useParams();
  console.log(id);
  const { demandsData, demandsLoading } = useDemands(id);
  const { userData } = useUser();
  const { companyProfileData } = useCompanyProfile(
    demandsData ? demandsData.company_id : ""
  );
  const [tag, setTag] = useState(demandsData?.tag || "");
    ...
    if (
      ...
      setMessageInfo({
        ...
        message: {
          ...
          receiver_id: demandsData.contact_id,
        },
      });
    } else {
      setPayPopup(true);
  ...
  useEffect(() => {
    const found = needsOptions_data[currentLanguage].find(
      (option) => option[1] === demandsData?.tag
    );
    if (found) setTag(found[0]);
  }, [currentLanguage, demandsData]);
      ...
      {demandsLoading ? (
        ...
        <div className="flex flex-col">
          ...
          <p className="text-xs2 text-input-line mt-2">
            {t("connect.demand.post_on")} {formatDate(demandsData.created_at)}{" "}
            {" | "}
            {t("connect.demand.valid_until")}{" "}
            {formatDate(demandsData.expired_at)}
          </p>
              ...
              <tbody>
                <tr>
                  <td className="text-toggle-color pb-2 w-24">
                    {t("connect.demand.compant_info.name")}
                  </td>
                ...
                <tr>
                  ...
                  <td className="text-db-table-text pb-2">
                    {formatDate(demandsData.expired_at) || "N/A"}
                  </td>
                </tr>
              </tbody>
            </table>
