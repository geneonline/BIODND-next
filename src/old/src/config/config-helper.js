...
export function getConfig(engine_name) {
  if (process.env.NODE_ENV === "test") {
    return {};
  }

  if (config[engine_name]?.engineName) return config[engine_name];
...
export function getTitleField(current_engineName) {
  // If no title field configuration has been provided, we attempt
  // to use a "title" field, if one exists
  return getConfig(current_engineName).titleField || "title";
}

export function getUrlField(current_engineName) {
  return getConfig(current_engineName).urlField;
}

export function getThumbnailField(current_engineName) {
  return getConfig(current_engineName).thumbnailField;
}

export function getFacetFields(current_engineName) {
  return getConfig(current_engineName).facets || [];
}

export function getSortFields(current_engineName) {
  return getConfig(current_engineName).sortFields || [];
}
...
export function buildSearchOptionsFromConfig(current_engineName) {
  const config = getConfig(current_engineName);
...
export function buildFacetConfigFromConfig(current_engineName) {
  const config = getConfig(current_engineName);
...
export function buildSortOptionsFromConfig(current_engineName) {
  const config = getConfig(current_engineName);
  return [
    {
      name: "Relevance",
...
export function buildAutocompleteQueryConfig(current_engineName) {
  const querySuggestFields = getConfig(current_engineName).querySuggestFields;
  if (
    !querySuggestFields ||
  ...
  return {
    suggestions: {
      types: {
        documents: {
          fields: getConfig(current_engineName).querySuggestFields,
        },
      },
    },
  };
