...
const PRIMARY_FIELD = "device_id";

const buildBackToSearchUrl = (searchParams) => {
  const queryString = searchParams.toString();
  return `/database/search/assets/device${
...
const DeviceDetail = () => {
  ...
  const infoRows = useMemo(() => {
    const rows = [
      {
        label: "Device Number",
        value: normalizeValue(data?.device_id),
      },
      {
        label: "Product Code",
        value: normalizeValue(data?.product_code),
      },
      {
        label: "Regulation Number",
        value: normalizeValue(data?.regulation_number),
      },
      {
        label: "Customs Declaration Number",
        value: normalizeValue(data?.customs_declaration_number),
      },
      {
        label: "Manufacturing Permit Number",
        value: normalizeValue(data?.manufacturing_permit_number),
      },
      {
        label: "Standard Device Name",
        value: normalizeValue(data?.device_name_standard),
      },
      {
      ...
      },
      {
        ...
        value: normalizeValue(data?.device_registry_T),
          ...
          {isLoading ? (
            ...
            <>
              <div className="bg-white flex flex-col gap-4 p-8 rounded-lg shadow-md">
                <div className="flex justify-between items-center">
                  <div className="flex gap-2 flex-wrap">
                    {data.device_class && (
                      <span className="bg-gray-100 px-2.5 py-1 rounded-lg text-sm text-gray-600">
                        {normalizeValue(data.device_class)}
                      </span>
                    )}
                  ...
                  <div className="flex gap-2">
                    <ShareButton
                      title={normalizeValue(data.device_name)}
                ...
                <h2 className="text-xl font-medium text-textColor-primary leading-160">
                  {normalizeValue(data.device_name)}
                </h2>
                <hr className="border-gray-300" />
            ...
            <div className="flex flex-col gap-4 items-start">
              ...
              <p className="text-textColor-Tertiary leading-160">
                We could not locate a device asset for this identifier. Return
                to the asset search page and try a different result.
              </p>
              <Link
                to={backToSearchUrl}
